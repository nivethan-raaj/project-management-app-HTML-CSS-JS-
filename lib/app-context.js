...
export function AppProvider({ children }) {
  ...
  const [onboardedEmails, setOnboardedEmails] = useState([]);
  ...
  const login = useCallback((email, password) => {
    const alreadyOnboarded = onboardedEmails.includes(email);
  ...
  const loginWithSocial = useCallback((provider) => {
    ...
    const alreadyOnboarded = onboardedEmails.includes(email);
    setUser({ name: names[provider] || "User", email, avatar: null, provider });
    if (alreadyOnboarded) {
      setOnboardingComplete(true);
      setCurrentPage("home");
    } else {
  ...
  const completeOnboarding = useCallback((profileData) => {
    ...
    setOnboardingComplete(true);
    if (user?.email) {
      setOnboardedEmails((prev) => prev.includes(user.email) ? prev : [...prev, user.email]);
    }
    setCurrentPage("home");
  }, [user]);
